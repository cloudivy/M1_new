...
# Step 1: Identify chainages with >10 leak events (adapted for Streamlit)
high_leak_chainages = []
st.info("🔍 Scanning all chainages for high leak activity...")
...
if high_leak_chainages:
    ...
    high_risk_df = pd.DataFrame({
        'Chainage (km)': [f"{c:.1f}" for c in high_leak_chainages],
        'Leak Events': [len(df_lds_IV[abs(df_lds_IV['chainage'] - c) <= tolerance]) for c in high_leak_chainages]
    })
    st.dataframe(high_risk_df, use_container_width=True)
    
    # Step 2: Plot only high-leak chainages (Streamlit version)
    st.subheader("📊 Detailed Analysis of High-Risk Chainages")
    
    for target_chainage_val in high_leak_chainages[:5]:  # Limit to top 5 to avoid overcrowding
        with st.expander(f"Chainage {target_chainage_val:.1f} ({len(df_lds_IV[abs(df_lds_IV['chainage'] - target_chainage_val) <= tolerance])} leaks)"):
            df_digging_filtered = df_manual_digging[
                abs(df_manual_digging['Original_chainage'] - target_chainage_val) <= tolerance
            ].copy()
            df_leaks_filtered = df_lds_IV[
                abs(df_lds_IV['chainage'] - target_chainage_val) <= tolerance
            ].copy()
            
            col1, col2, col3 = st.columns(3)
            col1.metric("🔵 Digging Events", len(df_digging_filtered))
            col2.metric("🔴 Leak Events", len(df_leaks_filtered))
            col3.metric("⚠️ Risk Ratio", f"{len(df_leaks_filtered)/max(len(df_digging_filtered),1):.1f}x")
            
            # Plot for this chainage
            plt.close('all')
            ...
            plt.title(f'Digging vs. Leak Events at Chainage {target_chainage_val:.1f} ±{tolerance}km', fontsize=16)
...
st.markdown("---")
st.caption("💡 Upload files → Adjust chainage → Click Analyze → View correlations! 🚨 High-risk chainages automatically identified!")
